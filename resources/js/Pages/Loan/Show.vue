        ...
        <main class="px-2 md:px-10 pt-4 pb-16">
            ...
            <div class="flex justify-between">
                <div class="md:w-1/3 mr-2">
                    <el-select @change="$inertia.get(route('loans.show', loanSelected))" v-model="loanSelected" clearable
                        filterable placeholder="Buscar" no-data-text="No hay órdenes registradas"
                        no-match-text="No se encontraron coincidencias">
                        <el-option v-for="item in loans" :key="item.id" 
                            :label="item.type === 'Otorgado' ? 'O-' + item.id?.toString().padStart(3, '0') + ' - ' + item.beneficiary_name
                            : 'R-' + item.id?.toString().padStart(3, '0') + ' - ' + item.lender_name"
                            :value="item.id">
                            <p class="flex items-center justify-between">
                                <span>{{ item.type === 'Otorgado' ? 'O-' + item.id?.toString().padStart(3, '0') + ' - ' + item.beneficiary_name 
                                        : 'R-' + item.id?.toString().padStart(3, '0') + ' - ' + item.lender_name }}</span>
                                <span class="text-gray-400 text-xs">${{ item.amount?.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</span>
                            </p>
                        </el-option>
                    </el-select>
                </div>
                ...
                <div class="flex items-center space-x-2">
                    <ThirthButton @click="$inertia.get(route('loans.edit', loan.id))" class="flex items-center space-x-1">
            ...
            <section class="flex space-x-5 mt-9 text-[15px]">
                <article class="w-1/3 grid grid-cols-2 gap-2 rounded-xl bg-[#F2F2F2] border border-grayD9 p-5">
                    ...
                    <p class="text-[#575757]">{{ loan.type === 'Otorgado' ? 'Nombre del beneficiario:' : 'Nombre del prestamista:'}}</p>
                    ...
                    <p v-if="loan.profitability">{{ loan.profitability }} {{ loan.profitability_mode === 'Porcentaje' ? '%' : '$' }}</p>
