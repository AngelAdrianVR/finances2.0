<template>
    <AppLayout title="Editar gasto recurrente">
        <main class="px-3 md:px-16 py-8">
            <Back />
            <form @submit.prevent="update" class="rounded-xl border border-grayD9 dark:border-gray-600 dark:bg-[#1E2424] lg:p-5 p-3 lg:w-2/3 xl:w-1/2 mx-auto mt-2 lg:grid lg:grid-cols-2 gap-3">
                <h1 class="font-bold ml-2 col-span-full mb-4 dark:text-gray-200">Editar gasto recurrente</h1>
...
data() {
    const form = useForm({
        amount: this.recurring_outcome.amount,
        category: this.recurring_outcome.category,
        concept: this.recurring_outcome.concept,
        payment_method: this.recurring_outcome.payment_method,
        periodicity: this.recurring_outcome.periodicity,
        description: this.recurring_outcome.description,
