    ...
    <AppLayout title="Editar gasto">
        <main class="px-3 md:px-16 py-8">
            ...
            <form @submit.prevent="update" class="rounded-xl border border-grayD9 lg:p-5 p-3 lg:w-2/3 xl:w-1/2 mx-auto mt-2 lg:grid lg:grid-cols-2 gap-3 shadow-lg">
                <h1 class="font-bold ml-2 col-span-full mb-4">Editar gasto</h1>
                ...
                <div>
                    ...
                    <el-input
                        ...
                        maxlength="50"
                        placeholder="Ej. Pago de teléfono"
                        show-word-limit
                        type="text"
                    />
...
data() {
    const form = useForm({
        ...
        concept: this.outcome.concept,
        created_at: this.outcome.created_at,
        payment_method: this.outcome.payment_method,
...
methods:{
    update() {
        this.form.put(route("outcomes.update", this.outcome.id), {
            onSuccess: () => {
                // message
                this.$message({
                    type: 'success',
