<template>
                    ...
                    <template #default="scope">
                        <div class="flex items-center space-x-2">
                            <el-tooltip v-if="scope.row.automatically_created" effect="dark"
                                content="El registro se ha realizado automáticamente porque el gasto es recurrente"
                                placement="top">
    ...
    <DialogModal :show="showDetailsModal" @close="showDetailsModal = false">
        ...
        <template #footer>
            <button @click="$inertia.get(route('outcomes.edit', itemToShow.id))"
    ...
    <DialogModal :show="showMassiveEditModal" @close="showMassiveEditModal = false" maxWidth="lg">
        <template #title>
            <h1 class="font-bold text-left">Editar gastos masivamente</h1>
        </template>
        <template #content>
            <form @submit.prevent="massiveUpdate" class="mb-32">
                ...
                <p class="text-gray37 text-sm mt-3">
                    Todos los gastos seleccionados <span class="text-primary">({{ $refs.multipleTableRef.value.length
...
export default {
    ...
    methods: {
        ...
        massiveUpdate() {
            this.form.post(route('outcomes.massive-update'), {
                onSuccess: () => {
