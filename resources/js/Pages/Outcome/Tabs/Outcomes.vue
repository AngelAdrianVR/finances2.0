                ...
                <el-table-column label="Monto" width="150">
                    <template #default="scope">
                        <div class="flex items-center space-x-2">
                            <el-tooltip v-if="scope.row.automatically_created" effect="dark" content="El registro se ha realizado automáticamente porque el gasto es recurrente" placement="top">
    ...
    <DialogModal :show="showDetailsModal" @close="showDetailsModal = false">
        <template #title>
            <div class="flex items-center justify-between">
                <p class="font-bold text-left mb-5">Detalles de gasto</p>
                <el-tooltip v-if="itemToShow.automatically_created" effect="dark" content="El registro se ha realizado automáticamente" placement="right">
                    <div class="inline-flex items-center rounded-full px-3 py-1 bg-[#fec0c0] text-red-600 text-sm">
        ...
        <template #footer>
            <button @click="$inertia.get(route('outcomes.edit', itemToShow.id))" class="size-9 border border-primary text-primary rounded-full flex justify-center items-center mr-2">
...
methods:{
    ...
    handleCommand(command) {
        ...
        if ( commandName === 'show' ) {
            ...
            this.$inertia.get(route('outcomes.' + commandName, rowId));
        }
    },
    async deleteSelections() {
        this.$confirm('¿Estás seguro que deseas continuar con la eliminación?', 'Confirmar', {
            ...
            try {
                const response = await axios.post(route('outcomes.massive-delete', {
                    outcomes: this.$refs.multipleTableRef.value
                }));
                ...
                if (response.status === 200) {
                    ...
                    // update list of outcomes
                    let deletedIndexes = [];
                    this.outcomes.data.forEach((income, index) => {
                    ...
                    for (const index of deletedIndexes) {
                        this.outcomes.data.splice(index, 1);
                    }
    ...
    handleRowClick(row) {
        this.itemToShow = row;
        this.showDetailsModal = true;
        // this.$inertia.get(route('outcomes.show', row.id));
