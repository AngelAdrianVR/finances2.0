<template>
    <AppLayout title="Calendario">
                            ...
                            <div v-for="reminder in reminders" :key="reminder.id">
                                <div class="mt-1" v-if="isRemaindDay(reminder, day)">
                                    <div @click.stop="handleShowReminderDetails(reminder)"
                                        ...
                                        :class="{
                                            'line-through': reminder.status === 'Registrado',
                                            'bg-[#E2FFD5]': reminder.type === 'Ingreso recurrente',
                                            'bg-[#CBEAF8]': reminder.type === 'Gasto fijo',
                                        }">
                                        <svg v-if="reminder.status === 'Pendiente'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                                        ...
                                        <p class="hidden md:block truncate md:max-w-12 lg:max-w-24">{{ reminder.title }}</p>
                                    </div>
                                </div>
                            </div>
                        </td>
        ...
        <el-drawer v-model="drawer" :size="drawerSize" >
            ...
            <template #default>
                <div>
                    <div class="flex items-center justify-between text-base font-bold border-b border-grayD9 pb-3">
                        <h2>{{ reminderSelected?.title }}</h2>
                        <h2 class="rounded-full px-4 py-1"
                            :class="{
                                'text-orange-600 bg-orange-100': reminderSelected?.status === 'Pendiente',
                                'text-green-600 bg-green-100': reminderSelected?.status === 'Registrado',
                                }">
                            {{ reminderSelected?.status }}
                        </h2>
                    </div>
