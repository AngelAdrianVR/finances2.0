...
function handleCommand(command) {
    ...
    if (action === 'show') {
        ...
        router.get(route('recurring-incomes.edit', id));
...
async function toggleStatus(rowId) {
    try {
        ...
        if (response.status === 200) {
            const idx = props.recurring_incomes.data.findIndex(item => item.id == rowId);
            if (idx !== -1) {
                props.recurring_incomes.data[idx].is_active = response.data.is_active;
            }
...
async function deleteSelections() {
    try {
        await ElMessageBox.confirm(
            'Estas seguro que deseas eliminar los ingresos recurrentes seleccionados?',
        ...
        const response = await axios.post(route('recurring-incomes.massive-delete'), {
            recurring_incomes: selectedRows.value,
        });

        if (response.status === 200) {
            ElMessage.success('Eliminados correctamente.');
            const deletedIds = new Set(selectedRows.value.map(r => r.id));
            props.recurring_incomes.data = props.recurring_incomes.data.filter(d => !deletedIds.has(d.id));
...
function tableRowClassName({ row }) {
    return row.is_active ? 'cursor-pointer' : 'cursor-pointer inactive-row';
}
</script>
...
<template>
    <div class="income-table-section">
        ...
        >
            ...
            <el-table-column prop="payment_method" label="Metodo" width="130">
                <template #default="{ row }">
                    {{ row.payment_method ?? '-' }}
                </template>
            </el-table-column>
            <el-table-column label="Creado" width="170">
                <template #default="{ row }">
    ...
    <!-- Detail dialog -->
    <el-dialog v-model="showDetail" title="Detalles del ingreso recurrente" width="500px" destroy-on-close>
            ...
            <div class="space-y-2">
                ...
                <div class="detail-row">
                    ...
                    <span
                        ...
                        :class="itemToShow.is_active
                            ? 'bg-[#EAF7EF] text-[#247A47]'
                            : 'bg-[#FCEAEA] text-[#B23636]'"
                    >
                        {{ itemToShow.is_active ? 'Activo' : 'Inactivo' }}
                    </span>
                </div>
            </div>
        </template>
        <template #footer>
            <el-button @click="router.get(route('recurring-incomes.edit', itemToShow?.id))" v-if="itemToShow">Editar</el-button>
