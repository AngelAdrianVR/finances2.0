<template>
    <main class="border border-grayD9 rounded-xl p-5 shadow hover:shadow-lg transition-all ease-linear duration-200 hover:-translate-y-1">
        ...
        <div v-else class="w-full flex items-center flex-wrap justify-center gap-10">
            ...
            <div>
                <h2 class="text-center text-black text-base font-semibold leading-relaxed pb-1">No hay información para mostrar</h2>
                <p class="text-center text-black text-sm font-normal leading-snug pb-4">Agrega un ingreso o gasto para <br />visualizar la gráfica </p>
                <div class="flex gap-3 mt-3">
                    <button class="custom-button" @click="$inertia.get(route('incomes.create'))">+ Ingreso</button>
                    <button class="custom-button" @click="$inertia.get(route('outcomes.create'))">+ Gasto</button>
                </div>
            </div>
            </div>
        </div>
