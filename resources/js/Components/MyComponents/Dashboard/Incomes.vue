<template>
    <main
        ...
        <section class="sm:grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 space-y-2 sm:space-y-0">
            <!-- Ventas -->
            <div @click="handleShowDetailIncomes('Ventas')"
                ...
                <div>
                    ...
                    <p class="text-base md:text-lg font-bold text-[#575757]">$ {{
                        sales.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</p>
                </div>
            </div>
            <!-- Intereses -->
            <div @click="handleShowDetailIncomes('Intereses')"
                class="border border-grayD9 rounded-lg p-4 cursor-pointer flex space-x-5 transition-all ease-linear duration-200 hover:scale-105">
                ...
                <div>
                    ...
                    <p class="text-base md:text-lg font-bold text-[#575757]">$ {{
                        intereses.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</p>
                </div>
            </div>
            <!-- Nómina -->
            <div @click="handleShowDetailIncomes('Nómina')"
                class="border border-grayD9 rounded-lg p-4 cursor-pointer flex space-x-5 transition-all ease-linear duration-200 hover:scale-105">
                ...
                <div>
                    ...
                    <p class="text-base md:text-lg font-bold text-[#575757]">$ {{
                        payroll.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</p>
                </div>
            </div>
            <!-- Prestación de servicios -->
            <div @click="handleShowDetailIncomes('Prestación de servicios')"
                class="border border-grayD9 rounded-lg p-4 cursor-pointer flex space-x-5 transition-all ease-linear duration-200 hover:scale-105">
                ...
                <div>
                    ...
                    <p class="text-base md:text-lg font-bold text-[#575757]">$ {{
                        service.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</p>
                </div>
            </div>
            <!-- Comisión -->
            <div @click="handleShowDetailIncomes('Comisión')"
                class="border border-grayD9 rounded-lg p-4 cursor-pointer flex space-x-5 transition-all ease-linear duration-200 hover:scale-105">
                ...
                <div>
                    ...
                    <p class="text-base md:text-lg font-bold text-[#575757]">$ {{
                        comision.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</p>
                </div>
            </div>
            <!-- Renta -->
            <div @click="handleShowDetailIncomes('Renta')"
                class="border border-grayD9 rounded-lg p-4 cursor-pointer flex space-x-5 transition-all ease-linear duration-200 hover:scale-105">
                ...
                <div>
                    ...
                    <p class="text-base md:text-lg font-bold text-[#575757]">$ {{
                        rent.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",") }}</p>
                </div>
            </div>
            <!-- Otros -->
            <div @click="handleShowDetailIncomes('Otro')"
                class="border border-grayD9 rounded-lg p-4 cursor-pointer flex space-x-5 transition-all ease-linear duration-200 hover:scale-105">
        ...
        <DialogModal :show="showIncomesDetailsModal" @close="showIncomesDetailsModal = false">
            ...
            <template #content>
                <div class="mt-4 max-h-80 overflow-auto">
                    <table v-if="incomes.some(item => item.category === categorySelected)"
...
export default {
    ...
    computed: {
        totalIncomes() {
            return this.sales + this.intereses + this.payroll + this.service + this.comision + this.rent + this.another;
        },
    },
    methods: {
        formatDate(date) {
