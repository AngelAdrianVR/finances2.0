<template>
    <div class="z-50 h-64 w-56 bg-white shadow-md border border-grayD9 absolute rounded-[10px]">
        <div
            ...
            <button @click="$emit('close')" class="absolute top-1 right-2 rounded-full hover:bg-gray-300 hover:text-red-600 size-5 text-xs text-black">
                <i class="fa-solid fa-xmark"></i>
            </button>
            <Link :href="route('profile.show')">
            <button
                class="absolute flex items-center justify-center size-5 rounded-[5px] top-1 left-2 text-[11px] text-gray99 bg-transparent">
        ...
        <div class="flex flex-col text-black text-left mt-11 mx-7">
            ...
            <p class="flex items-center space-x-2">
                ...
                <span class="text-[10px]">{{ $page.props.auth.user.employee_properties?.phone }}</span>
            </p>
        </div>
