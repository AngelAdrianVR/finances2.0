<template>
  <div v-if="pagination?.links?.length > 3" class="overflow-x-auto flex items-center justify-between my-1">
    ...
    <div class="flex flex-1 items-center justify-between">
      <div>
        <nav class="isolate inline-flex -space-x-px rounded-lg text-[11px]" aria-label="Pagination">

          <template v-for="(link, key) in pagination?.links">
            <div :key="key" v-if="link.url == null"
              class="rounded-md relative inline-flex items-center border bg-gray-100 cursor-pointer hover:bg-gray-200 hover:border-primary px-2 py-1 text-[11px] font-medium text-gray-600 border-gray-400 focus:z-20 "
              v-html="link.label">

            </div>

            <Link :key="'link-' + key" v-else :href="link?.url" v-html="link?.label"
              class="relative inline-flex items-center border px-3 py-1 text-[11px] lg:text-md rounded-md"
              :class="link.active ? 'z-10 bg-emerald-50 border-2 border-primary text-primary font-bold' : 'bg-gray-50 border-gray-400 text-gray-700 hover:bg-gray-200 hover:border-primary'" />
          </template>
